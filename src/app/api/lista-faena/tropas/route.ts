...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      const existing = await tx.listaFaenaTropa.findUnique({
        where: {
          listaFaenaId_tropaId_corralId: { listaFaenaId, tropaId, corralId: corralId || null }
        }
      })
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      // Find the tropa from lista (with optional corralId)
      const existing = await tx.listaFaenaTropa.findUnique({
        where: {
          listaFaenaId_tropaId_corralId: { listaFaenaId, tropaId, corralId: corralId || null }
        }
